...
class InfoMessageHeader():
    ...
    def hardware_info(self):
        return self.message_id == 0 and self.secondary_message_id == 4
    ...
    def packet_size(self):
        try:
            switcher_packet_size = {
                    ...
                    (0, 4):  12, # sizeof(struct HardwareInfo) in MainFirmware  
...
class HardwareInfo():
    def __init__(self, data):
        if len(data) != 8:
            raise InvalidMessageSizeError("Invalid hardware version info size", data)
        self.raven_maverick_n = data[0]
        self.platform = data[1]
        self.features = data[2:6]
        
    @property
    def raven_maverick(self):
        return self.raven_maverick_n
    @property
    def platform(self):
    ...
    def __str__(self):
        model = "MAVERICK" if self.raven_maverick==2 else "RAVEN"
