...
import script_lib.get_usb_info_with_output as usbView


run_once = 0

def performCycle(ser, port, time_delay, count, usbDataFile):
    for counter in range (0, count):
        print("cycles remaining = ", count - counter)
        usbView.usbData(usbDataFile)
        time.sleep(0.2)
        testCycle(ser)
...
if __name__ == "__main__":
    ...
    outputFilePath_4 = os.path.join(os.path.dirname(__file__), "logs", dateTimeStamp, "usbData" + ".txt")
    ...
    with serial.Serial(args.device, args.baud) as ser, open(outputFilePath_1, mode='wb') as outputBinFile:
        ...
        try:
            ...
            while True:
                ...
                outputBinFile.flush()
                if run_once == 0:
                    performCycle(ser, args.device, args.time_delay, args.count, outputFilePath_4)
