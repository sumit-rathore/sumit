...
enum LexPmState
{
    ...
    PM_POWER_DOWN,                      // 4 Host in power down mode
    PM_HOST_LINK_TRAINING,              // 5 Link training is working
    PM_HOST_LINK_TRAINED_WAIT_MSA,      // 6 Link trained and wait MSA
    PM_HOST_LINK_TRAINED,               // 7 Link is trained
    PM_HOST_LINK_TRAINED_WAIT_MCA,      // 8 Link up waiting for MCA
    PM_VIDEO_WAITING_FOR_REX,           // 9 Waiting until REX is ready to get Video
    PM_VIDEO_FLOWING,                   // 10 Video is flowing from LEX to REX
    PM_NO_VIDEO,                        // 11 Video stop. disconnected/resolution change/host shutdown
    PM_DISABLE_PENDING,                 // 12 Transition status before going to Disable state
    PM_ERROR,                           // 13 Error state
    NUM_STATES_POLICY_MAKER             // 14
};
...
struct LexPmStateFlags
{
    ...
    uint32_t rexNewLinkParams   : 1;    // Rex link-training changed link parameters
};
...
static enum LexPmState LexPmPowerDownHandler(enum LexPmEvent event, enum LexPmState currentState)               __attribute__((section(".lexatext")));
...
static void SendStreamParameters()                                      __attribute__((section(".lexatext")));
...
static void LexCheckIfCapIsValid(void)                                  __attribute__((section(".lexftext")));
...
static const EventHandler pmStateTable[] =
{
    ...
    [PM_POWER_DOWN]                 = LexPmPowerDownHandler,
...
void DP_LEX_IcmdPrintAllStatusFlag(void)
{
    ...
    ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, DP_REXNEWLNKPARA,   lexPmContext.stateFlags.rexNewLinkParams);
...
//#################################################################################################
// PM_POWER_DOWN handler
//      Local sink is in power down mode (Sleep)
//
// Parameters:
// Return:
// Assumptions: parameter 'currentState' could be next state for LEX_AUX_EVENT_ENTER
//              or LEX_AUX_EVENT_EXIT
// The Sink device has asserted HPD, and sufficiently enabled the AUX CH to at least monitor incoming
// AUX CH differential signals. The Main-Link RX disabled.
//#################################################################################################
static enum LexPmState LexPmPowerDownHandler(enum LexPmEvent event, enum LexPmState currentState)
{
    enum LexPmState nextState = currentState;

    if (event == LEX_AUX_EVENT_ENTER)
    {
        LexLtStateSendEventWithNoData(LEX_LT_POWER_DOWN);
        LexPmUpdateVideoStatus(VIDEO_IN_RESET);
    }
    else if (event == LEX_AUX_PWD_UP)                   // just send event to link training state machine
    {
        DP_Lex_StartRexPowerDownTimer();
        LexLtStateSendEventWithNoData(LEX_LT_POWER_UP);
    }
    else if (event == LEX_AUX_POWER_UP_TRAINED)         // Power up with link trained
    {
        nextState = PM_HOST_LINK_TRAINED_WAIT_MSA;
    }
    else if (event == LEX_AUX_HOST_REQUEST_TRAINING)    // Power up with link unlock
    {
        nextState = PM_HOST_LINK_TRAINING;
    }
    else if (event == LEX_AUX_RX_MONITOR_INFO)
    {
        if(lexPmContext.stateFlags.rexNewMonitor || lexPmContext.stateFlags.rexNewLinkParams)
        {
            lexPmInitContext.requestNewLinkTraining = true;
            nextState = PM_HOST_LINK_TRAINING;
        }
...
static enum LexPmState LexPmLinkTrainingHandler( enum LexPmEvent event, enum LexPmState currentState)
{
    ...
    {
        if(RexEdidChanged(&lexPmInitContext.sinkParameters.edidCache[0]) || lexPmContext.stateFlags.rexNewLinkParams)
        {
...
static enum LexPmState LexPmVideoFlowingHandler( enum LexPmEvent event, enum LexPmState currentState)
{
    ...
    else if (event == LEX_AUX_PWD_UP)
    {
        ilog_DP_COMPONENT_2(ILOG_MINOR_ERROR, PM_UNHANDLED_EVENT, event, currentState);
    }
    else
    {
        nextState = LexPmCommonHandler(event, currentState);
...
static enum LexPmState LexPmCommonHandler(enum LexPmEvent event, enum LexPmState currentState)
{
    ...
    switch(event)
    {
        ...
        case LEX_AUX_PWR_DOWN:
            if (currentState >= PM_HOST_LINK_TRAINED)
            {
                DP_Lex_StartRexPowerDownTimer();
                nextState = PM_POWER_DOWN;
            }
            break;
...
static void LexPmCpuMsgReceivedEventHandler(uint8_t subType, const uint8_t *msg, uint16_t msgLength)
{
    ...
    switch(subType)
    {
        case AUX_MSG_SINK_PARAMETERS:
            ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, AUX_LEX_GET_SINK_PARAMETER, 1);
        ...
        case AUX_MSG_STREAM_PARAMETERS_ACK:
            break;
...
// Parameters:
// Return:
// Assumptions: rexNewLinkParams set means only CAP is updated
//
//#################################################################################################
static void LexPmHandleMonitorInfo(void)
{
    ...
    LexCheckIfCapIsValid();
    DPCD_LoadReceiverCapCache();

    lexPmContext.stateFlags.rexNewMonitor = RexEdidChanged(&lexPmInitContext.sinkParameters.edidCache[0]);
...
//
//#################################################################################################
static void SendStreamParameters()
{
    struct AUX_DownstreamCpuMessage msg = {
        .type = AUX_MSG_STREAM_PARAMETERS,
        .msgBuffer = &lexPmContext.linkAndStreamParameters.streamParameters,
        .msgLength = sizeof(lexPmContext.linkAndStreamParameters.streamParameters)
    };

    LexSendCpuMessageToRex(&msg);
    lexPmContext.stateFlags.rexWaitHostInfo = false;
    ilog_DP_COMPONENT_0(ILOG_USER_LOG, PM_SENT_STREAM_PARAMS);
}
...
static void LexPmClearContext()
{
    ...
    lexPmContext.stateFlags.rexNewLinkParams = false;
...
{
    // Poll the no video steam here. If noVideoStream is zero, then reset the stream extractor.
...
//#################################################################################################
// Checks if the Sink Params are Valid
// Parameters:
// Return:
// Assumption:
//
//#################################################################################################
static void LexCheckIfCapIsValid(void)
{
    ...
    iassert_DP_COMPONENT_2(bw == BW_1_62_GBPS || bw == BW_2_70_GBPS || bw == BW_5_40_GBPS,
                            AUX_INVALID_BW, bw, __LINE__);

    iassert_DP_COMPONENT_1(lc == LANE_COUNT_1 || lc == LANE_COUNT_2 || lc == LANE_COUNT_4,
                            AUX_INVALID_LC, lc);

    lexPmContext.stateFlags.rexNewLinkParams = RexLinkParamsChanged(bw, lc);
}
