...
ICMD_FUNCTIONS_CREATE(TOPLEVEL_COMPONENT)
    ICMD_FUNCTIONS_ENTRY(xmodem_new_image, "Send a new image over xmodem and then run it", void)
    ICMD_FUNCTIONS_ENTRY(PrintSwVersion, "Print out the current software version and build date/time", void)
    ICMD_FUNCTIONS_ENTRY(TOPLEVEL_printDeviceType, "Prints whether this device is a LEX or a REX.", void)
    ICMD_FUNCTIONS_ENTRY(TOPLEVEL_getEcoStatus, "Prints the status of ECO Flag, (1 = ECO detected).", void)
    ICMD_FUNCTIONS_ENTRY(TOPLEVEL_DEBUG_ASSERT, "CAUSE AN ASSERT IN GE FOR DEBUGGING", uint8)
