...
//Internal function
struct _ipool_free_list * _IPOOL_init (void * mempool, uint32 elementSize, uint32 numberOfElements);
...
// static inline uint32 PREFIX_poolPtrToIndex(elementType *);
// static inline elementType * PREFIX_poolIndexToPtr(uint32 index);
// static inline uint32 PREFIX_poolGetNumOfUsedElements();
// static inline uint32 PREFIX_poolGetNumOfFreeElements();
...
static uint32 ipool_prefix ## _ipoolNumOfUsedElements;                                      \
                                                                                            \
/*                                                                                      */  \
...
{                                                                                           \
    ...
    element = CAST(old_head, struct _ipool_free_list *, elementType *);                     \
    ipool_prefix ## _ipoolNumOfUsedElements += (element == NULL) ? 0 : 1;                   \
    return element;                                                                         \
}                                                                                           \
                                                                                            \
/*                                                                                      */  \
...
/*                                                                                      */  \
/* @return -    uint32 index into memory pool array                                     */  \
/*                                                                                      */  \
/* @note   -    The return value can't be larger than the # of elements in the pool     */  \
/*              So it is safe to assume uint8, when there are less than 256 entries     */  \
/*              So it is safe to assume uint16, when there are less than 65536 entries  */  \
/*                                                                                      */  \
static inline uint32 ipool_prefix ## _poolPtrToIndex(elementType * e) __attribute__((always_inline, const)); \
static inline uint32 ipool_prefix ## _poolPtrToIndex(elementType * e)                       \
{                                                                                           \
    if (e == NULL)                                                                          \
    {                                                                                       \
        return 0;                                                                           \
    ...
    {                                                                                       \
        return (    (uint32)(e)                        /* Address of element */             \
                - ((uint32)(&ipool_prefix ## _pool[0]))) /* Address of mempool */           \
...
/*                                                                                      */  \
static inline elementType * ipool_prefix ## _poolIndexToPtr(uint32 index) __attribute__((always_inline, const)); \
static inline elementType * ipool_prefix ## _poolIndexToPtr(uint32 index)                   \
{                                                                                           \
    if (index == 0)                                                                         \
    {                                                                                       \
        return NULL;                                                                        \
...
/* @note   -                                                                            */  \
/*                                                                                      */  \
static inline uint32 ipool_prefix ## _poolGetNumOfUsedElements()                            \
{                                                                                           \
...
/* @note   -                                                                            */  \
/*                                                                                      */  \
static inline uint32 ipool_prefix ## _poolGetNumOfFreeElements()                            \
{                                                                                           \
