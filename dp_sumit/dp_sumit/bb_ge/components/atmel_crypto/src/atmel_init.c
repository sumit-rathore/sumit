///////////////////////////////////////////////////////////////////////////////
///
///   Icron Technology Corporation - Copyright 2012
///
///
///   This source file and the information contained in it are confidential and
///   proprietary to Icron Technology Corporation. The reproduction or
///   disclosure, in whole or in part, to anyone outside of Icron without the
///   written approval of a Icron officer under a Non-Disclosure Agreement, or
///   to any employee of Icron who has not previously obtained written
///   authorization for access from the individual responsible for the source
///   code, will have a significant detrimental effect on Icron and is
///   expressly prohibited.
///
///////////////////////////////////////////////////////////////////////////////
//
//!   @file  - atmel_init.c
//
//!   @brief - Contains the main initialization function for this module
//
//!   @note  -
//
///////////////////////////////////////////////////////////////////////////////

/***************************** Included Headers ******************************/
#include "atmel_crypto_loc.h"
#include <grg.h> // For getting platformId (IE isSpartan)

/************************ Defined Constants and Macros ***********************/

/******************************** Data Types *********************************/

/***************************** Local Variables *******************************/

/************************ Local Function Prototypes **************************/

/************************** Function Definitions *****************************/

/**
* FUNCTION NAME: ATMEL_init()
*
* @brief  - Initialization for this module
*
* @return - void
*
* @note   -
*
*/
void ATMEL_init
(
    // If non-NULL check if the Atmel chip is locked, and then call this function
    void (*completionHandler)(boolT dataAndOtpZonesLocked, boolT configZoneLocked)
)
{
    ilog_SetLevel(ILOG_MAJOR_EVENT, ATMEL_CRYPTO_COMPONENT);
    _ATMEL_i2cInit();
    _ATMEL_macInit();

   if (completionHandler)
   {
       iassert_ATMEL_CRYPTO_COMPONENT_1(
           ATMEL_isChipLocked(completionHandler),
           INVALID_COND, __LINE__);
   }
}

