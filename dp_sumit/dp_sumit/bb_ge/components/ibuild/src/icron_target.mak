...
targ_icron_clean_files=icron_header target.elf target.bin flash_writer.bin ilog icmd icmdresp icomponent $(SPECTAREG_XML_FILES) $(IDESIGN_XML_FILES)
...
targ_build2 = $(QUIET_ICRON)\
	...
	$(OBJCOPY) -O binary $(targ_dir)/target.elf $(targ_dir)/target.bin && \
	$(if $(SPECTAREG_XML_FILES), $(CP) $(foreach file, $(SPECTAREG_XML_FILES), $(TOP_LEVEL_COMPONENTS_DIR)/spectareg/etc/$(file)) $(targ_dir) &&) \
	$(if $(IDESIGN_XML_FILES), $(CP) $(foreach file, $(IDESIGN_XML_FILES), $(IDESIGN_XML_DIR)/$(file)) $(targ_dir) &&) \
	echo -e -n "version:3\n"\
	"project:$(if $(IBUILD_BUILDING_TEST_HARNESS),blackbird_test,blackbird)\n"\
	"ilog:ilog\n"\
	"icmd:icmd\n"\
	"icomponent:icomponent\n"\
	"icmdresp:icmdresp\n"\
	...
	$(if $(HOBBES_DEFAULT_ICMD_BUTTON9), "hobbes_default_icmd_button9: $(HOBBES_DEFAULT_ICMD_BUTTON9)\n")\
	$(IBUILD_ICRON_HEADER_SPECTAREG_BLOCK)\
	$(IBUILD_ICRON_HEADER_IDESIGN_BLOCK)\
	| $(_IBUILD_SED_REMOVING_LEADING_AND_TRAILING_WHITESPACE) > $(targ_dir)/icron_header && \
	tar cjf $@ -C $(targ_dir) icron_header $(if $(IBUILD_BUILDING_TEST_HARNESS),, flash_writer.bin) target.bin ilog icmd icmdresp icomponent $(SPECTAREG_XML_FILES) $(IDESIGN_XML_FILES)
