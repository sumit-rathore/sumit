...
#define MESSAGE_SIZE (ATMEL_SECRET_KEY_SIZE+ATMEL_MAC_CHALLENGE_SIZE+sizeof(messageTail))
/******************************** Data Types *********************************/
struct macStatus {
    ...
    void (*completionHandler)(enum ATMEL_processState atmelState);
...
)
{
    ...
    const struct ATMEL_I2COperation runMacParams =
    {
        .opCode                   = MAC,
        .param1                   = MAC_MODE,
        .param2                   = PARAM2_LSB + (PARAM2_MSB << 8),
        .writeDataSize            = ATMEL_MAC_CHALLENGE_SIZE,
        .writeData                = challenge,
        .readDataSize             = SHA256_DIGEST_SIZE,
        .completionHandler        = &atmel_doneHwMac,
        .userPtr                  = &macState,
        .needSleep                = true,
        .operationExecutionTime   = MACTime + 1
    };

    ret = _ATMEL_submitI2cOperation(runMacParams);
...
)
{
    ...
    if((data == NULL) || (dataSize != SHA256_DIGEST_SIZE))
    {
        // Don't assert and gives back failure mesage for retry
        ilog_ATMEL_CRYPTO_COMPONENT_3(ILOG_MAJOR_ERROR, HW_MAC_DONE_INVALID_ARGS, (uint32)data, dataSize, (uint32)userPtr);
        (*macState.completionHandler)(ATMEL_AUTH_FAIL);
    }
    else
    {
...
static void atmel_doSwMac()
{
    ...
    enum ATMEL_processState atmelState = ATMEL_AUTH_PASS;
    ...
    memcpy( &message[ATMEL_SECRET_KEY_SIZE],
            macState.challenge,
            ATMEL_MAC_CHALLENGE_SIZE);
    memcpy( &message[ATMEL_SECRET_KEY_SIZE+ATMEL_MAC_CHALLENGE_SIZE],
            messageTail,
            sizeof(messageTail));
    ...
    {
        if (swDigest[index] == macState.digest[index])
        {
            // bytes match :)
        }
        else
        {
            // bytes differ :(
            atmelState = ATMEL_AUTH_FAIL;
        }
    }
    (*macState.completionHandler)(atmelState);
}
