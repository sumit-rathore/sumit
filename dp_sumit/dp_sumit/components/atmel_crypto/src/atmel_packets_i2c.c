...
// #if 1
#define _ATMEL_GO_IDLE_CMD (0x02) // idle command, TempKey and RNG seed register are retained
// #else
#define _ATMEL_GO_SLEEP_CMD (0x01) // sleep command, all volatile state is reset
// #endif
...
static struct
{
    ...
    TIMING_TimerHandlerT wakeupWriteTimer;
    ...
    boolT needSleep;    // Used to determine sleep mode setting after read done
} atmel_i2c __attribute__((aligned(4)));

// Atmel I2C device characteristic
const struct I2cDevice i2cDeviceAtmel =
{
    .deviceAddress = ATMEL_I2C_DEVICE_ADDRESS,
    .speed = ATMEL_I2C_SPEED,
    .port = I2C_MUX_CORE
};

/************************ Local Function Prototypes **************************/
static void atmel_crc(void *param1, void *param2)           __attribute__((section(".atext")));
static void atmel_start_wakeup(
        ...
        void
#endif
    ) __attribute__((noinline, section(".atext")));
static void atmel_wokeup(void)                              __attribute__((section(".atext")));
static void atmel_write(void);
static void atmel_writeDone(boolT success)                  __attribute__((section(".atext")));
static void atmel_executionTimeElapsed(void)                __attribute__((section(".atext")));
static void atmel_readDone(uint8 * data, uint8 byteCount)   __attribute__((section(".atext")));
static void atmel_idleDone(boolT success)                   __attribute__((section(".atext")));
...
static void atmel_assertHelper(uint32 line)                 __attribute__((noinline, noreturn, section(".atext")));
static void atmel_debugDumpState(void);

// Recovery functions
static void atmel_startRecovery(void)                       __attribute__((section(".atext")));
static void atmel_recoveryWokeup(void)                      __attribute__((section(".atext")));
static void atmel_recoveryInactive(boolT success)           __attribute__((section(".atext")));

/************************** Function Definitions *****************************/

/**
* FUNCTION NAME: ATMEL_i2cInit()
*
...
void ATMEL_i2cInit()
{
    ...
    atmel_i2c.wakeupWriteTimer = TIMING_TimerRegisterHandler(
        &atmel_write, false, 3);
...
{
    ifail_ATMEL_CRYPTO_COMPONENT_1(INVALID_COND, line);
...
)
{
    if (atmel_i2c.chipState != ATMEL_ASLEEP)
    {
        ...
        ilog_ATMEL_CRYPTO_COMPONENT_1(ILOG_MAJOR_ERROR, ATMEL_SUBMIT_I2C_SYS_BUSY, atmel_i2c.chipState);
        return false;
    }
    ...
    *CAST((&atmel_i2c.writeBuffer[0]), uint8 *, uint32 *) = bufStart; // This populates entries 0 to 3 of the buffer
    ...
    CALLBACK_Run(atmel_crc, NULL, NULL);
...
*
*/
static void atmel_crc(void *param1, void *param2)
{
    uint16 crc;
    uint8 crc_msb;
    uint8 crc_lsb;
...
)
{
    ...
    {
        I2C_Wake(&i2cDeviceAtmel, atmel_wokeup);
    }
    else
    {
...
* @return - void
*
* @note   - This sends wakeup signal to the Atmel chip
*
*           See the top of the file for the process of a complete i2c operation
*/
static void atmel_wokeup(void)
{
    ...
    TIMING_TimerStart(atmel_i2c.wakeupWriteTimer);
}

/**
* FUNCTION NAME: atmel_write()
*
* @brief  - Write message after wairing Wakeup time (2.5ms at least, check 6.1.1 Device is Asleep - Atmel datasheet)
*
* @return - void
*
...
static void atmel_executionTimeElapsed(void)
{
    ...
    I2C_ReadAsync( &i2cDeviceAtmel,
                  atmel_i2c.readBuffer,
                  i2cBytesToRead,
                  atmel_readDone );
}
...
static void atmel_readDone(uint8 * data, uint8 byteCount)
{
    ...
    else
    {
        ...
        if(atmel_i2c.needSleep)
        {
            atmel_i2c.simpleCmd = _ATMEL_GO_SLEEP_CMD;
        }
        else
        {
            atmel_i2c.simpleCmd = _ATMEL_GO_IDLE_CMD;
        }
...
* @return - void
*
* @note   - This will call back the completion handler from the __ATMEL_submitI2cOperation() caller
*
...
{
    if(atmel_i2c.retryCount > ATMEL_I2C_RETRY_LIMIT)
    {
        // Communication fail
        // If maximum number of recovery fails, stop communication and call callback
        // In case of no ATMEL chipset, assert will happen in initialization step
        ilog_ATMEL_CRYPTO_COMPONENT_0(ILOG_MINOR_ERROR, MAX_I2C_RETRY_EXCEEDED);
        atmel_i2c.chipState = ATMEL_ASLEEP;     // Step initialization
        (*atmel_i2c.completionHandler)(NULL, 0, atmel_i2c.userPtr); // No databuffer, 0 data read size
        return;
    }
    ...
    I2C_Wake(&i2cDeviceAtmel, atmel_recoveryWokeup);
}
