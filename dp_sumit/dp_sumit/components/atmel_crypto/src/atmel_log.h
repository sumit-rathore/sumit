...
ILOG_CREATE(ATMEL_CRYPTO_COMPONENT)
    ...
    ILOG_ENTRY(INVALID_ATMEL_BLOCK, "Invalid Atmel OTP block %d\n")
    ILOG_ENTRY(READ_SLOT_FAILED, "Read slot failed, read %d bytes instead of 32 bytes\n")
    ILOG_ENTRY(WRITE_SLOT_FAILED, "Write slot/block failed, read %d status bytes, data[0] is 0x%x\n")
    ...
    ILOG_ENTRY(ATMEL_SUBMIT_I2C_SYS_BUSY, "Submit i2c op, but system is busy at step:%d\n")
    ...
    ILOG_ENTRY(ATMEL_WRITE_DATA_SLOT_OR_OTP_BLOCK_ICMD_COMPLETE, "ICMD write of data slot / OTP block completed\n")
    ...
    ILOG_ENTRY(RUN_NONCE_CALLED_WHILE_IN_PROGRESS, "run Nonce called while in progress\n")
    ILOG_ENTRY(RUN_NONCE_CALLED, "Run Nonce called, secretKey is at 0x%x, challenge is at 0x%x, completionHandler is at 0x%x\n")
    ILOG_ENTRY(ENCRYPT_STEP, "Current Encrypt Step is %d\n")
    ILOG_ENTRY(READ_32BYTES, "ATMEL READ 32 BYTES: 0x%04x, 0x%04x, 0x%04x\n")
    ILOG_ENTRY(ATMEL_FIFO_FULL, "ATMEL FIFO TASK FULL \n")
    ILOG_ENTRY(MAC_FAILED, "MAC challenge failed\n")
    ILOG_ENTRY(MAC_PASSED, "MAC challenge passed\n")
    ILOG_ENTRY(ATMEL_NOT_EXIST, "Atmel ATSHA204 does not exist\n")
    ILOG_ENTRY(ATMEL_TASK_TIME, "Atmel Task current tick is = %d\n")
    ILOG_ENTRY(ATMEL_VALIDATION, "Atmel Validation Start\n")
    ILOG_ENTRY(ATMEL_INVALID_STATE, "Atmel invalid state\n")
ILOG_END(ATMEL_CRYPTO_COMPONENT, ILOG_DEBUG)

#endif // #ifndef ATMEL_LOG_H
