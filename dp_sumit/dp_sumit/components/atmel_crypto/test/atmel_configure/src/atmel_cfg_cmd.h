...
ICMD_FUNCTIONS_CREATE(TEST_HARNESS_COMPONENT)
    ICMD_FUNCTIONS_ENTRY_FLASH(writeDataByte, "Write data byte to the Atmel data zone. Args: slotNumber, addr, data", uint8, uint8, uint8)
    ICMD_FUNCTIONS_ENTRY_FLASH(writeCfgByte, "Write configuration byte to the Atmel configuration zone. Args: addr, data", uint8, uint8)
    ICMD_FUNCTIONS_ENTRY_FLASH(writeCfgSlotConfig, "Write the configuration for a particular data zone slot. Args: slotNumber, slotCfg", uint8, uint16)
    ICMD_FUNCTIONS_ENTRY_FLASH(burnAndLock, "Burn all writes to the Atmel chip, and lock the chip", void)
    ICMD_FUNCTIONS_ENTRY_FLASH(showConfig, "Show the current configuration settings", void)
    ICMD_FUNCTIONS_ENTRY_FLASH(showData, "Show the current data zone for a particular slot", uint8)
