...
static struct {
    ...
    uint8_t   lastIteration;
} _TIMING;

/************************ Local Function Prototypes **************************/
static void _TIMING_AssertNullPointer(uint32_t line) __attribute__((noreturn, noinline));
#ifdef GE_PROFILE
static void performProfile(uint8_t timerIdx, LEON_TimerValueT time);
#endif
...
(
    void (*timeoutCallback)(void),  // Function to call when the timer expires
    bool periodic,                 // If true, the timer will be restarted automatically after
                                    // each timeout
    uint32_t timeoutInMs              // How many milliseconds should pass prior to calling the
                                    // timeoutCallback
)
{
    static uint8_t timersInUse = 0;
    TIMING_TimerHandlerT handler;
#ifndef BB_ROM
    iassert_TIMING_COMPONENT_2(
        timersInUse < MAX_TIMERS_AVAILABLE, TIMING_MAXIMUM_HANDLERS_REGISTERED, (uint32_t)timersInUse, (uint32_t)timeoutCallback);

    ilog_TIMING_COMPONENT_3(ILOG_USER_LOG, TIMING_NUMBER_HANDLERS_REGISTERED, (uint32_t)timersInUse, MAX_TIMERS_AVAILABLE, (uint32_t)timeoutCallback);
#endif    //BB_ROM
...
*
*/
static void _TIMING_AssertNullPointer(uint32_t line)
{
#ifndef BB_ROM
    ifail_TIMING_COMPONENT_1(NULL_TIMER_ARG_LINE, line);
#endif    //BB_ROM
...
*/
bool TIMING_TimerEnabled(TIMING_TimerHandlerT handler)
{
    if (handler == NULL)
    {
        _TIMING_AssertNullPointer(__LINE__);
...
*/
uint32_t TIMING_TimerGetTimeout(TIMING_TimerHandlerT handler)
{
    if (handler == NULL)
    {
        _TIMING_AssertNullPointer(__LINE__);
...
{
    const uint32_t curTimerTick = _TIMING.tickCounter; // only read the global variable once
    uint8_t i;
#ifdef TIMING_TIMERS_TO_CHECK_PER_ISR
    uint8_t timersToCheck = TIMING_TIMERS_TO_CHECK_PER_ISR;
#endif
...
//
// Parameters:
//      timerIdx    - index into timer handler to profile
//      time        - before timer ran timestamp
// Return:
// Assumptions:
//#################################################################################################
static void performProfile(uint8_t timerIdx, LEON_TimerValueT time)
{
    uint32_t duration = LEON_TimerCalcUsecDiff(time, LEON_TimerRead());
    if (duration == 0)
    {
        duration = 1;
    }
    if (duration > _TIMING_handlers[timerIdx].profile.longestRun)
    {
        _TIMING_handlers[timerIdx].profile.longestRun = duration;
    }
    if ((duration < _TIMING_handlers[timerIdx].profile.shortestRun) || (_TIMING_handlers[timerIdx].profile.shortestRun == 0))
    {
        _TIMING_handlers[timerIdx].profile.shortestRun = duration;
    }
    uint8_t idx = _TIMING_handlers[timerIdx].profile.lastRunIdx;
    idx = (idx + 1) & (NUM_LAST_RUNS - 1);
    _TIMING_handlers[timerIdx].profile.lastRuns[idx] = duration;
    _TIMING_handlers[timerIdx].profile.lastRunIdx = idx;
}
#endif // GE_PROFILE
