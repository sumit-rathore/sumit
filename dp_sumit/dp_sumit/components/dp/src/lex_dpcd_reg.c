...
//          Also, make sure that BW and LC are not Zero
//#################################################################################################
bool RexLinkParamsChanged(uint8_t rexBw, uint8_t rexLc)
{
    iassert_DP_COMPONENT_2(rexBw == BW_1_62_GBPS || rexBw == BW_2_70_GBPS || rexBw == BW_5_40_GBPS,
                            AUX_INVALID_BW, rexBw, __LINE__);

    iassert_DP_COMPONENT_1(rexLc == LANE_COUNT_1 || rexLc == LANE_COUNT_2 || rexLc == LANE_COUNT_4,
                            AUX_INVALID_LC, rexLc);
