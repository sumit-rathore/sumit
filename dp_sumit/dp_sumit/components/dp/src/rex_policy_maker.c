...
struct RexPmStateFlags
{
    ...
    uint32_t gotNewStreamParams         : 1;    // got new Stream Params from LEX (cleared after every link down)
    uint32_t gotNewLinkParams           : 1;    // got new Link Params from LEX (cleared after every link down)
...
struct RexPmInitContext                 // Context that needs to be cleared when it restarts DP
{
    ...
    struct LinkAndStreamParameters newLinkAndStreamParameters;
    uint16_t edidIndex;
    ...
    bool gotLinkAndStreamParamters;     // Not to miss an event REX_AUX_DP_RX_HOST_INFO
...
static bool HostLinkParamsChanged(void)                                                                     __attribute__((section(".rexatext")));
static void UpdateAudioData(const uint8_t *edidBaseBlock)                                                   __attribute__((section(".rexatext")));
static void loadDefaultLinkParams(void)                                                                     __attribute__((section(".rexatext")));
...
void DP_REX_IcmdPrintAllStatusFlag(void)
{
    ...
    ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, DP_GOTNEWSTRMPARAMS, rexPmContext.stateFlags.gotNewStreamParams);
    ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, DP_GOTNEWLINKPARAMS, rexPmContext.stateFlags.gotNewLinkParams);
    ...
    ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, DP_GOTSTREAMPARAM,   rexPmInitContext.gotLinkAndStreamParamters);
...
static enum RexPmState RexPmLinkTrainedNoVideoHandler(enum RexPmEvent event, enum RexPmState currentState)
{
    ...
    else if (event == REX_AUX_DP_RX_HOST_INFO)
    {
        ...
        if(rexPmContext.stateFlags.gotNewLinkParams)
        {
            rexPmContext.stateFlags.gotNewLinkParams = false;
            newState = PM_MONITOR_LINK_TRAINING;
        }
        else
        {
...
static void RexPmCpuMsgReceivedEventHandler(uint8_t subType, const uint8_t *msg, uint16_t msgLength)
{
    ...
    switch (subType)
    {
        case AUX_MSG_LINK_AND_STREAM_PARAMETERS:
            memcpy((uint8_t *)&rexPmInitContext.newLinkAndStreamParameters, msg, msgLength);

            rexPmInitContext.gotLinkAndStreamParamters = true;
            rexPmContext.stateFlags.gotNewStreamParams = HostStreamParamsChanged();
            rexPmContext.stateFlags.gotNewLinkParams = HostLinkParamsChanged();
            CPU_COMM_sendSubType(CPU_COMM_TYPE_AUX, AUX_MSG_LINK_AND_STREAM_PARAMETERS_ACK);
...
{
    if(!rexPmInitContext.gotLinkAndStreamParamters)
    {
        loadDefaultLinkParams();
    }
...
static enum ProcessEdidBaseBlockResult ProcessEdidBaseBlock(void)
{
    ...
    if (edidBaseBlock[EDID_EXTENSION_FLAG_ADDR] > 1)
    {
        ...
        rexPmInitContext.edidExtBlkIndex = edidBaseBlock[EDID_EXTENSION_FLAG_ADDR];
...
// Assumptions:
//#################################################################################################
static void loadDefaultLinkParams(void)
{
    ...
    rexPmInitContext.sinkParameters.receiverCapCache[MAX_LINK_RATE] = rexBandwidth;
    rexPmInitContext.sinkParameters.receiverCapCache[MAX_LANE_COUNT] =
                ((rexPmInitContext.sinkParameters.receiverCapCache[MAX_LANE_COUNT] & ~MAX_LANE_COUNT_MASK) | rexLaneCount);
...
//#################################################################################################
// Compare stream parameters with current values
//
// Parameters:
...
{
    struct DP_StreamParameters *newStreamParams = &rexPmInitContext.newLinkAndStreamParameters.streamParameters;
    ...
    // Log that status of stream parameters being changed
    ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, AUX_GOT_NEW_STREAM_INFO, streamParamChanged);
...
//#################################################################################################
// Compare link parameters with current values
//
// Parameters:
// Return: This function returns True only if the stream parameters have changed
//
...
//
//#################################################################################################
static bool HostLinkParamsChanged()
{
    // If BW and LC are supposed to be isolated between LEX and REX,
    // assume that the link params have not changed and continue
    if(dpConfigPtr->enableLcBwIsolation)
    {
        return false;
    }

    const uint8_t oldBw = rexPmInitContext.linkAndStreamParameters.linkParameters.bw;
    const uint8_t oldLc = rexPmInitContext.linkAndStreamParameters.linkParameters.lc;

    const uint8_t newBw = rexPmInitContext.newLinkAndStreamParameters.linkParameters.bw;
    const uint8_t newLc = rexPmInitContext.newLinkAndStreamParameters.linkParameters.lc;

    bool linkParamChanged = ((oldBw != newBw) || (oldLc != newLc));

    // Log that status of link parameters being changed
    ilog_DP_COMPONENT_1(ILOG_MAJOR_EVENT, AUX_GOT_NEW_LINK_INFO, linkParamChanged);

    if(linkParamChanged)
    {
        rexPmInitContext.linkAndStreamParameters.linkParameters.bw = newBw;
        rexPmInitContext.linkAndStreamParameters.linkParameters.lc = newLc;
    }

    return (linkParamChanged);
}
