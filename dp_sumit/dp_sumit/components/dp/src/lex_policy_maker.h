...
enum LexPmEvent
{
    ...
    LEX_AUX_AUDIO_MUTE_STATUS_CHANGE,           // 22   Status of audio mute has changed
    LEX_AUX_STREAM_ERROR_DETECTED,              // 23   A stream error has been detected (must be in VIDEO_FLOWING state)
    LEX_AUX_REX_READY_FOR_MCA,                  // 24   Rex is ready for MCA handling
    LEX_AUX_RETRAIN_REQUEST,                    // 25   Requested for re-link training due to bad link-quality
    LEX_AUX_START_DIAGNOSTIC,                   // 26   Event to restart Link Training and do system diagnostic
    LEX_AUX_POOR_LINK_QUALITY,                  // 27   LexCheckLinkQuality has declared bad DP link due to BER errors
};
