...
/*                                                                              */
/* FUNCTION NAME: UartfifoClear()                                                */
/*                                                                              */
/* @brief  - Clears the given FIFO                                         */
/*                                                                              */
/* @param - fifo - pointer to the fifo structure to work with                   */
/*                                                                              */
/* @return - nothing                                                            */
/*                                                                              */
void  UartfifoClear(UartFifoT *fifo)
{
    // initialize the fifo to empty, make sure the buffer is in a known state
    memset(fifo->fifo, 0, fifo->fifoSize);
    fifo->fifoReadIndex = fifo->fifoWriteIndex = 0;
}
...
/*                                                                              */
/* FUNCTION NAME: UartfifoCopy()                                                */
/*                                                                              */
/* @brief  - memcpy to the fifo                                                 */
/*                                                                              */
/* @param - fifo - pointer to the fifo structure to work with                   */
/* @param - buffer - the item pointer to store in the fifo                      */
/* @param - dataLength - total number of bytes to be written to fifo            */
/*                                                                              */
/* @return - void                                                               */
/*                                                                              */
/* @note   - UNSAFE: Always call fifoFull or fifoSpaceAvail before calling      */
/*           fifoWrite, as fifoWrite doesn't check if there is space available  */
/*         - assumes fifo size is a power of 2                                  */
/*         - assumes fifo size is greater than dataLength                       */
/*                                                                              */
void UartfifoCopy(UartFifoT *fifo, const uint8_t *buffer, uint16_t dataLength)
{
    const uint32_t fifo_mask = fifo->fifoSize - 1;
    uint16_t cpySize = MIN(dataLength, fifo->fifoSize - fifo->fifoWriteIndex);

    memcpy(&fifo->fifo[fifo->fifoWriteIndex], buffer, cpySize);
    fifo->fifoWriteIndex += cpySize;
    fifo->fifoWriteIndex &= fifo_mask;

    if(dataLength > cpySize)                    // in case of circular fifo reach the index end
    {
        buffer += cpySize;                      // update buffer pointer
        cpySize = dataLength - cpySize;         // remaining data to be sent
        memcpy(&fifo->fifo[fifo->fifoWriteIndex], buffer, cpySize);
        fifo->fifoWriteIndex += cpySize;
    }
}
...
/*                                                                              */
/* FUNCTION NAME: UartfifoBytesReceived()                                                    */
/*                                                                              */
/* @brief  - Returns the number of received bytes in the fifo                   */
/*                                                                              */
/* @param - fifo - pointer to the fifo structure to work with                   */
/*                                                                              */
/* @return - Number of bytes waiting to be processed                            */
/*                                                                              */
/* @note   -                                                                    */
/*                                                                              */
uint16_t UartfifoBytesReceived(UartFifoT *fifo)
{
    return (fifo->fifoSize - UartfifoSpaceAvail(fifo) - 1);
}
        ...
        uint16_t length)
{
    ...
    if ((fifo->fifoSize - UartfifoSpaceAvail(fifo)) >= length)
    {
        ...
        else
        {
            ...
            if (length <= bytesCopied) // no wrapping case
            {
                memcpy(buffer, &(fifo->fifo[fifo->fifoReadIndex]), length);
            }
            else // wrapping case
        ...
        const uint32 fifo_mask = fifo->fifoSize - 1;
