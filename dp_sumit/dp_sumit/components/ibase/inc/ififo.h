...
//    private:
//        element_type * fifo;
//        uint32_t fifoWriteIndex;
//        uint32_t fifoReadIndex;
//        uint32_t fifoSize;
//    public:
//        ififo(uint32_t fifoSizeArg);
//        ~ififo(void);
//        bool fifoEmpty(void);
//        bool fifoFull(void);
//        uint32_t fifoSpaceAvail(void);
//        uint32_t fifoSpaceUsed(void);
...
// Future enhancements
//--------------------
// 1) TODO: Could use uint8_t or uint16_t for size and index pointers to save space, pass in as extra argument
// 2) Could be changed to real libary calls that operate on a struct {
//    uint32_t size; uint32_t writeIndex; uint32_t readIndex; uint8_t fifo[_size_]};
...
static inline bool          fifo_prefix ## _fifoEmpty(void)            __attribute__((always_inline));\
static inline bool          fifo_prefix ## _fifoFull(void)             __attribute__((always_inline));\
static inline uint32_t         fifo_prefix ## _fifoSpaceAvail(void)       __attribute__((always_inline));\
static inline uint32_t         fifo_prefix ## _fifoSpaceUsed(void)        __attribute__((always_inline));\
...
static inline element_type * fifo_prefix ## _fifoReadPtr(void)          __attribute__((always_inline));\
...
static inline void           fifo_prefix ## _fifoFlush(void)            __attribute__((always_inline));\
static struct {                                                                             \
...
/*                                                                              */          \
/* @return - bool : true/false.  Answers question "Is the fifo empty?"          */          \
/*                                                                              */          \
static inline bool fifo_prefix ## _fifoEmpty(void)                                          \
{                                                                                           \
...
/*                                                                              */          \
/* @return - bool : true/false.  Answers question "Is the fifo full?"           */          \
/*                                                                              */          \
static inline bool fifo_prefix ## _fifoFull(void)                                           \
{                                                                                           \
...
/*                                                                              */          \
static inline uint32_t fifo_prefix ## _fifoSpaceAvail(void)                                 \
{                                                                                           \
...
/*                                                                              */          \
static inline uint32_t fifo_prefix ## _fifoSpaceUsed(void)                                  \
{                                                                                           \
    ...
    {                                                                                       \
        ...
        return (uint32_t)(spread);                                                          \
    }                                                                                       \
    else                                                                                    \
    {                                                                                       \
        /* spread indicates the negative of how many elements are used          */          \
        return (uint32_t)(fifo_size + spread);                                              \
    }                                                                                       \
}                                                                                           \
/*                                                                              */          \
...
{                                                                                           \
    ...
    {                                                                                       \
        ...
        const uint32_t fifo_mask = fifo_size - 1;                                           \
...
{                                                                                           \
    ...
    return readElement;                                                                     \
                                                                                           ...
                                                                                           \
/*                                                                              */          \
/* FUNCTION NAME: fifoReadPtr()                                                 */          \
/*                                                                              */          \
/* @brief  - Reads an element off the fifo                                      */          \
/*                                                                              */          \
/* @return - The pointer of element read                                        */          \
/*                                                                              */          \
/* @note   - UNSAFE: Always call fifoEmpty before calling fifoRead              */          \
/*           As fifoRead doesn't check if there is anything on the fifo         */          \
/*                                                                              */          \
/*           TODO: Could change name to fifoReadUnsafe to emphasize this        */          \
/*                                                                              */          \
static inline element_type * fifo_prefix ## _fifoReadPtr(void)                              \
{                                                                                           \
    element_type *readElement = &fifo_prefix.fifo[fifo_prefix.fifoReadIndex];               \
                                                                                            \
    /* Update read index pointer, and handle the wrap if it reaches the end */              \
...
/*                                                                              */          \
/* FUNCTION NAME: fifoFlush()                                                   */          \
/*                                                                              */          \
/* @brief  - Clears the fifo                                                    */          \
/*                                                                              */          \
/* @return - void                                                               */          \
/*                                                                              */          \
/* @note   - For efficiency, this function only zeros the fifo's read and       */          \
/*           write indices, not its backing array.                              */          \
static inline void fifo_prefix ## _fifoFlush(void)                                          \
{                                                                                           \
                                                                                            \
    fifo_prefix.fifoWriteIndex = 0;                                                         \
    fifo_prefix.fifoReadIndex = 0;                                                          \
