...
static uint32_t * xmodemBuf1;
static uint32_t * xmodemBuf2;
static volatile enum intXStates xmodemIrqState;
static volatile bool nextBufReady;
static volatile bool xmodemIrqDone;
static struct xmodemState xmodemIrqStateStruct;

/************************ Local Function Prototypes **************************/
static void rxByte(uint8_t);
...
*
*/
bool XMODEM_InterruptModeReceive
(
    uint32_t * (*rxDataHandlerFunction)(uint32_t * buf, uint32_t bufSize),// a handler to pass the xmodem bytes recieved successfully to
    uint32_t * buffer1,                                               // the 1st buffer to work with
    uint32_t * buffer2                                                // the 2nd buffer to work with
)
{
    bool loopAgain;
...
nextBufReadyisSet:
    ...
    UART_SetRxHandler(NULL);  // no Rx Handler set, anymore
    LEON_UnlockIrq(flags);
...
static void rxByte
(
    uint8_t byte  // the byte recieved in the uart interrupt
)
{
