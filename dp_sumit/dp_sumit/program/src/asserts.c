...
void LEON_UninitializedISR_C_helper1(uint32_t PC, uint32_t nPC) __attribute__ ((noreturn));
...
void LEON_DataStoreErrHandler(void) __attribute__ ((noreturn));
...
void LEON_UninitializedISR_C_helper1(uint32_t PC, uint32_t nPC)
{
...
void LEON_DataStoreErrHandler(void)
{
    ilog_TOPLEVEL_COMPONENT_0(ILOG_FATAL_ERROR, DATA_STORE_ERR_TRAP);
